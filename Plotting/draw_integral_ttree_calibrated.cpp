...
//   eta_edges.push_back(1.6);  //--- exclude
  ...
  if (argc>=10) {
    num_max_labberblade = atoi(argv[9]);
  }
  ...
  if (argc>=13) {
    want_to_use_the_smeared = atoi(argv[12]);
  }
  ...
  std::string fileCalibration_BPIX = "../Calibration/scale_BPIX_pixels_run_ranges.txt ";
  if (argc>=3) {
    fileCalibration_BPIX = argv[2];
    
    std::cout << " fileCalibration BPIX = " << fileCalibration_BPIX << std::endl;
    
    std::ifstream file (fileCalibration_BPIX); 
    std::string buffer;
    float value;
    ...
    while(!file.eof()) {
      ...
      if (buffer != "" && buffer.at(0) != '#'){ ///---> save from empty line at the end!
        ...
        // and "num_ladderbladeId" into "iladderblade"
        ilayer = ilayer -1 - min_layer ; // counting starts from 1
        iladderblade = iladderblade -1 ; // counting starts from 1
        //----
  ...
  std::string fileCalibration_FPIX = "../Calibration/scale_FPIX_pixels_run_ranges.txt ";
  if (argc>=4) {
    fileCalibration_FPIX = argv[3];
    
    std::cout << " fileCalibration FPIX = " << fileCalibration_FPIX << std::endl;
    
    std::ifstream file (fileCalibration_FPIX); 
    std::string buffer;
    float value;
...
//               whatToDraw = variable_dedx+ std::to_string(iHit) + "[best_track] >>+ " + name.Data();
              whatToDraw = variable_dedx+ std::to_string(iHit) + "[best_track] * " + map_calibration_BPIX[iRun][ilayer][iEdge][iladderblade] + " >>+ " + name.Data();
            ...
            if ( ladderbladeId.at(iladderblade) <= map_layer_max_ladders_FPIX[layerId.at(ilayer)] ) {  // optimization  
              ...
              name = Form ("h_iRun_%d__ilayer_%d__iEdge_%d__ladderblade_%d__dedxById_FPIX_data", iRun, layerId.at(ilayer), iEdge, ladderbladeId.at(iladderblade));   
//               whatToDraw = variable_dedx+ std::to_string(iHit) + "[best_track] >>+ " + name.Data();
              whatToDraw = variable_dedx+ std::to_string(iHit) + "[best_track] * " + map_calibration_FPIX[iRun][ilayer][iEdge][iladderblade] + " >>+ " + name.Data();
  ...
  for (int ilayer = 0; ilayer<layerId.size(); ilayer++) {
    for (int iEdge = 0; iEdge<eta_edges.size()-1; iEdge++) {    
      ...
      name = Form ("plots_summary_calibrated/cc_summary_layer_%d_eta_%d_BPIX.root" , layerId.at(ilayer), iEdge); 
      cc_summary->SaveAs(name.Data());
      ...
      cc_summary->SaveAs(name.Data());
      name = Form ("plots_summary_calibrated/cc_summary_layer_%d_eta_%d_FPIX.root" , layerId.at(ilayer), iEdge); 
      cc_summary->SaveAs(name.Data());
