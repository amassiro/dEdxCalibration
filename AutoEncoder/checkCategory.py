...
def getDataFromFileWithCut(fileinfo, branchlist, cut_variable, cut_value) :
  ...
  for entry in tree :
    ...
    if getattr(entry, cut_variable) == cut_value :
      ...
      for branch in branchlist :
        data[counter][bridx] = getattr(entry, branch)
        #if data[counter][bridx] == -1 :
          #print " data[counter][", branch, "] = " , data[counter][bridx]
...
for name, name_file in sigfiles.items():
  data_sigs[name] = getDataFromFileWithCut(name_file, brlist, 'tk_category', 1)
...
model = load_model("xtracks_NN_category.h5")
model.summary()
